...
export async function apply(ctx, config = {}, internals = {}) {
  ...
  const service = internals.service ?? createPocketService({
    ...
    // 隧道就绪回调：Quick 每次开启轮换共享密码；Named 使用设备认证，不再使用公网 PIN。
    onTunnelReady: (mode) => {
      if (mode === 'named') {
        logger.info('dsh-pocket: named tunnel ready — device authentication active | 命名隧道就绪（设备认证已启用）');
        return null;
      }
  ...
  const disposeRpc = installPocketRpc(ctx, {
    ...
    // 恢复出厂设置：清空设置、重设随机密码并撤销全部 Named 设备。
    resetPocket: async () => {
      const pins = resetPocketState();
