...
button,.button-link{width:100%;border:0;background:var(--navy);color:#fff;padding:13px 16px;font:700 14px/1.2 inherit;cursor:pointer;transition:transform .15s ease,background .15s ease}.button-link{display:block;text-align:center;text-decoration:none;margin-top:18px}button:hover,.button-link:hover{background:#123653}button:active,.button-link:active{transform:translateY(1px)}button:focus-visible,.button-link:focus-visible{outline:3px solid rgba(33,184,137,.45);outline-offset:3px}button:disabled{opacity:.58;cursor:wait}
...
</style></head><body><main><div class="rail" aria-hidden="true"></div><section class="content"><div class="eyebrow">${eyebrow}</div>${body}<div class="action">${action}</div><div id="error" class="error" hidden role="alert"></div>${foot ? `<div class="foot">${foot}</div>` : ''}</section></main><script src="/pocket-auth/auth-v2.js" defer></script></body></html>`;
}
...
export function deviceLoginPage({ deviceCount = 0, paired = false } = {}) {
  if (!deviceCount) {
    return shell({
      ...
      body: '<h1>这台电脑尚未批准设备</h1><p>请回到电脑本机，在 DSH Pocket 设置中添加并批准一台设备。</p>',
      action: '<button type="button" disabled>等待电脑批准设备</button>',
    });
  }
  if (!paired) {
    return shell({
      title: '浏览器尚未配对',
      eyebrow: 'Device credential required',
      body: '<h1>此浏览器尚未配对</h1><p>仅知道设备密码不能登录。请回到电脑本机生成二维码，用当前浏览器重新完成配对。</p>',
      action: '<button type="button" disabled>需要重新配对</button>',
      foot: '更换浏览器或清除浏览数据后，都需要重新配对。',
    });
  }
  return shell({
    title: '输入设备密码',
    eyebrow: 'Approved device',
    body: '<h1>输入 Pocket 设备密码</h1><p>这是配对当前浏览器时设置的独立密码。DSH Pocket 不会显示或找回密码。</p>',
    action: '<form id="login-form" class="stack" autocomplete="off"><label for="login-password">设备密码</label><input id="login-password" name="pocket-device-secret" type="password" minlength="6" maxlength="256" autocomplete="off" autocapitalize="none" spellcheck="false" required><button id="login-button" type="submit">进入 DSH</button></form>',
    foot: '不建议让浏览器保存此密码。页面连续没有前台活动满 10 分钟后，需要重新输入。',
  });
}
...
export function devicePairPage() {
  return shell({
    ...
    body: '<h1>申请绑定当前浏览器</h1><p>填写后还需要回到电脑本机点击“允许”。二维码和密码都不能单独授予访问权限。</p>',
    action: '<form id="pair-form" class="stack" autocomplete="off"><label for="device-name">设备名称</label><input id="device-name" name="device-name" maxlength="80" autocomplete="off" placeholder="例如：荣耀 50 Chrome" required><label for="device-password">设备密码</label><input id="device-password" name="pocket-new-secret" type="password" minlength="6" maxlength="256" autocomplete="off" autocapitalize="none" spellcheck="false" placeholder="至少 6 个字符" required><label for="device-password-confirm">再次输入设备密码</label><input id="device-password-confirm" name="pocket-new-secret-confirm" type="password" minlength="6" maxlength="256" autocomplete="off" autocapitalize="none" spellcheck="false" required><button id="pair-button" type="submit">提交配对申请</button></form><div id="pair-complete" class="complete" hidden><strong>配对申请已提交</strong><p>请回到电脑，在“等待电脑批准”中点击“允许”。批准后回到此页面，点击下方按钮并输入设备密码。</p><a class="button-link" href="/">电脑已批准，进入 DSH</a></div>',
    foot: '不建议让浏览器保存此密码。配对申请 5 分钟后失效；过期后请回电脑重新生成二维码。',
  });
}
