...
import { POCKET_RPC_CHANNEL, POCKET_ADMIN_RPC_CHANNEL, POCKET_ENDPOINTS, redactStatus } from '../client/api.js';
...
export function installPocketRpc(ctx, { service, deviceAuth = null, log = console, desktop = false, runUpdate = null, restart = null, restartNotice = null, getToken = null, getLanToken = null, refreshLanToken = null, getLanAuthEnabled = null, setLanAuthEnabled = null, getLanEnabled = null, setLanEnabled = null, getLanIpOverride = null, setLanIpOverride = null, getPinCustom = null, setCustomPin = null, getTunnelConfig = null, setTunnelConfig = null, resetPocket = null }) {
  if (!ctx?.connection?.rpc?.handle) {
    log.warn?.('dsh-pocket: DSH Host Connection RPC unavailable — settings tab disabled | 无 Connection RPC，设置页不可用');
    return () => {};
  }
  const deviceOnlyEndpoints = new Set([
    POCKET_ENDPOINTS.deviceStatus,
    POCKET_ENDPOINTS.devicePairingStart,
    POCKET_ENDPOINTS.deviceApprove,
    POCKET_ENDPOINTS.deviceReject,
    POCKET_ENDPOINTS.deviceRevoke,
  ]);
  const handler = async (endpoint, payload = {}, signal, admin = true) => {
    if (signal?.aborted) return fail('cancelled', 'The request was cancelled.');
    // 设备管理只能在电脑本机执行（admin channel）；代理侧已拒绝非 loopback 的
    // /dsh-pocket-admin 请求，这里兜底拒绝普通通道上的 device.* 端点。
    if (!admin && deviceOnlyEndpoints.has(endpoint)) {
      return fail('bad-request', '设备管理只能在电脑本机执行 | device management is local-only');
    }
    ...
    const statusPayload = async () => {
      ...
      return ok({
        ...
        lanEnabled: getLanEnabled?.() ?? false, // 缺省与 settings.mjs 的默认关闭语义一致
    ...
    try {
      ...
      if (endpoint === POCKET_ENDPOINTS.deviceStatus) {
        if (!deviceAuth) return fail('bad-request', '设备认证不可用 | device authentication unavailable');
        return ok(deviceAuth.status());
      }
      if (endpoint === POCKET_ENDPOINTS.devicePairingStart) {
        ...
        const { qrDataUrl } = await import('./service.mjs');
  ...
  const disposeAdmin = ctx.connection.rpc.handle(POCKET_ADMIN_RPC_CHANNEL, (endpoint, payload, signal) => handler(endpoint, payload, signal, true));
  const disposePublic = ctx.connection.rpc.handle(POCKET_RPC_CHANNEL, (endpoint, payload, signal) => handler(endpoint, payload, signal, false));
  return () => {
    try { disposePublic?.(); } catch {}
    try { disposeAdmin?.(); } catch {}
  };
}
