...
//
// 当前项：
//   - lanEnabled        局域网访问总开关（**默认关闭**）：开启前必须手动确认；关闭后局域网扫码/链接直接失效（代理拒绝局域网 Host）
...
// 局域网访问本身**默认关闭**（三通道语义统一：默认关、手动开、开启必读声明）；
// 开启后默认还要输 8 位密码（lanAuthEnabled 默认开启）——用户可关闭密码，关闭后局域网扫码直连
// （仅同一网络内的设备能访问，HTTP 明文），公网不受影响（永远要密码）。
...
/**
 * 局域网访问总开关：**默认关闭**（文件缺失/损坏都视为关闭）。
 * 三通道语义统一：默认关、手动开启、开启时必读安全声明（LAN 开启弹窗见 client）。
 * 注意：这是 v2.10.3 之后的行为变更——老用户升级且从未动过开关时，LAN 会转为关闭（breaking change）。
 */
