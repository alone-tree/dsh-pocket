...
const ARGON2_OPTIONS = Object.freeze({ type: argon2.argon2id, memoryCost: 19 * 1024, timeCost: 2, parallelism: 1 });
...
function cleanPassword(value) {
  ...
  if (password.length < 6 || password.length > 256) throw new Error('设备密码应为 6-256 个字符 | device password must be 6-256 characters');
  return password;
}
...
function tokenHash(value) { return createHash('sha256').update(String(value ?? ''), 'utf8').digest('base64url'); }
function publicView(record) {
  return { id: record.id, name: record.name, createdAt: record.createdAt, lastLoginAt: record.lastLoginAt ?? null };
}
...
function noContent(res, headers = {}) { res.writeHead(204, { 'cache-control': 'no-store', ...headers }); res.end(); }
function deviceCookie(value, { clear = false } = {}) {
  return `${DEVICE_CREDENTIAL_COOKIE}=${value}; Secure; HttpOnly; SameSite=Strict; Path=/; ${clear ? 'Max-Age=0' : `Max-Age=${DEVICE_COOKIE_MAX_AGE_SECONDS}`}`;
}
...
/** Named Tunnel 专用：长期浏览器凭据 + 每设备密码 + 短会话。 */
export async function createDeviceAuth({
  ...
  passwords = null,
  rateLimits = { login: { limit: 30, windowMs: 60_000 }, pairing: { limit: 10, windowMs: 5 * 60_000 } },
  log = console,
} = {}) {
  ...
  const state = stored?.version === 2 && Array.isArray(stored.devices) ? stored : { version: 2, devices: [] };
  // 兼容旧实现字段名：其含义本来就是最后登录时间。
  for (const device of state.devices) {
    if (device.lastLoginAt == null && device.lastUsedAt != null) device.lastLoginAt = device.lastUsedAt;
    delete device.lastUsedAt;
  }
  ...
  function cleanup() {
    ...
    for (const [id, row] of sessions) if (t - row.lastActivityAt > sessionIdleMs) sessions.delete(id);
  }
  function trimOldest(map, limit) { while (map.size >= limit) map.delete(map.keys().next().value); }
  function consumeRateLimit(map, policy, req) {
    ...
    let row = map.get(key);
    if (!row) { trimOldest(map, 1024); row = { startedAt: t, count: 0 }; map.set(key, row); }
    row.count += 1;
  ...
  function pairingByToken(token) {
    ...
    const row = id && secret && !extra ? pairings.get(id) : null;
    if (!row || row.used || row.expiresAt <= now() || !secureEqual(row.secret, secret)) throw new Error('配对链接已失效 | pairing link has expired');
    return row;
  }
  ...
  function sessionFromRequest(req) {
    ...
    const row = id ? sessions.get(id) : null;
    if (!row || now() - row.lastActivityAt > sessionIdleMs || !state.devices.some((item) => item.id === row.deviceId)) {
      if (id && row) sessions.delete(id);
      return null;
    }
    return { id, ...row };
  }
  ...
  const api = {
    ...
    status() {
      ...
      try { publicOrigin = origin(); } catch {}
      return {
        configured: Boolean(publicOrigin), publicOrigin, legacyStateDetected,
        deviceCount: state.devices.length,
        devices: state.devices.map(publicView),
        pending: [...pending.values()].map((row) => ({ id: row.id, name: row.device.name, requestedAt: row.requestedAt, expiresAt: row.expiresAt })),
      };
    },
    beginPairing() {
      cleanup();
      ...
      pairings.set(id, row);
      return { id, url: `${publicOrigin}/pocket-pair#pair=${encodeURIComponent(`${id}.${secret}`)}`, expiresAt: row.expiresAt };
    },
    submitPairing({ token, name, password }) {
      return mutate(async () => {
        ...
        const device = {
          id: randomId(18), name: cleanDeviceName(name), tokenHash: tokenHash(rawDeviceToken),
          passwordHash: await hashPassword(cleanPassword(password)), createdAt: requestedAt,
          lastLoginAt: null, failedAttempts: 0, lockedUntil: null,
        };
        const request = { id: randomId(18), device, requestedAt, expiresAt: requestedAt + PAIRING_TTL_MS };
        pending.set(request.id, request);
        return { pendingId: request.id, name: device.name, expiresAt: request.expiresAt, deviceToken: rawDeviceToken };
      });
    },
    approvePending(id) {
      return mutate(async () => {
    ...
    rejectPending(id) {
      if (!pending.delete(String(id ?? ''))) throw new Error('设备申请不存在或已过期 | device request not found or expired');
      return true;
    },
    revokeDevice(id) {
      return mutate(async () => {
    ...
    login({ req, password }) {
      return mutate(async () => {
        ...
        await rejectLoginAttempt(device, String(password ?? ''));
        ...
        sessions.set(sessionId, { deviceId: device.id, createdAt: now(), lastActivityAt: now() });
        return { sessionId, deviceToken: rawDeviceToken, device: publicView(device) };
      });
    },
    hasApprovedDevice(req) { return Boolean(deviceFromRequest(req)); },
    authorizeRequest(req) { return sessionFromRequest(req); },
    ...
    async reset() {
      state.devices = [];
      pairings.clear(); pending.clear(); sessions.clear();
      await save();
    },
    async handleHttp(req, res) {
      ...
      try {
        if (req.method === 'POST' && path === '/pocket-auth/login') {
          ...
          const result = await api.login({ req, password: (await readJsonBody(req)).password });
          json(res, 200, { ok: true, redirect: '/', device: result.device }, { 'set-cookie': [deviceCookie(result.deviceToken), sessionCookie(result.sessionId)] });
          return true;
        }
        if (req.method === 'POST' && path === '/pocket-auth/activity') {
          if (!api.recordUserActivity(req)) return json(res, 401, { error: 'unauthorized' });
          noContent(res);
          return true;
        }
        ...
        if (req.method === 'POST' && path === '/pocket-pair/submit') {
          ...
          json(res, 200, { ok: true, request: { pendingId: result.pendingId, name: result.name, expiresAt: result.expiresAt } }, { 'set-cookie': deviceCookie(result.deviceToken) });
          return true;
        }
        return false;
      } catch (error) {
        ...
        json(res, rateLimited ? 429 : loginFailure ? 401 : 400, { error: loginFailure ? GENERIC_LOGIN_ERROR : (error?.message ?? String(error)) });
        return true;
      }
    },
  };
