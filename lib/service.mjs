...
export function createPocketService({
  ...
  const publicChannelForHost = (host) => {
    ...
    if (tunnel?.mode === 'quick') return hostname.endsWith('.trycloudflare.com') ? 'quick' : 'reject';
    if (tunnel?.mode === 'named') {
      const configured = String(getTunnelConfig?.()?.hostname ?? '').trim().toLowerCase();
      return configured && hostname === configured ? 'named' : 'reject';
    }
    return 'reject';
  };
  ...
  // 只有 Named（固定域名）参与自动恢复：Quick 是临时通道，重启后域名本就会变新，
  // 自动恢复等于替用户重新做「暴露到公网」的决定（且 PIN 可能轮换），必须每次手动开启。
  ...
  function persistAutoTunnel(mode) {
    ...
    autoStateQueue = autoStateQueue.then(async () => {
      try {
        ...
        await writeFile(autoStatePath, JSON.stringify({ at: Date.now(), mode: mode === 'named' ? 'named' : 'quick' }), 'utf8');
      } catch { /* 忽略 */ }
    });
  ...
  return {
      ...
      for (let p = port; p < port + 10; p++) {
        try {
          proxy = await createProxy({
            ...
            ...(publicChannelForHost ? { publicChannelForHost } : {}),
    ...
    async startTunnel() {
      ...
      tunnelPromise = (async () => {
        try {
          ...
          // 记录「隧道开启中（含通道）」，供重启后自动恢复（issue #11；仅 Named 恢复）
          void persistAutoTunnel(tunnel.mode);
    ...
    /** 启动时自动恢复公网隧道（issue #11）。仅 Named（固定域名）恢复；Quick 每次需手动开启。 */
    async restoreTunnelIfNeeded() {
      if (!autoStatePath || tunnel || tunnelPromise) return;
      let mode = null;
      try {
        mode = JSON.parse(await readFile(autoStatePath, 'utf8'))?.mode ?? null;
      } catch { return; } // 无标记/坏 JSON → 不恢复
      if (mode !== 'named') return; // Quick 标记仅作记录，不自动恢复
      try {
        await this.startTunnel();
        console.log('dsh-pocket: named tunnel auto-restored | 已自动恢复固定域名隧道（随机域名不自动恢复）');
      } catch (err) {
