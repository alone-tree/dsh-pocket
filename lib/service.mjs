...
import { startNamedTunnel } from './tunnel.mjs';
...
export function createPocketService({
  ...
  injectHtml,
  /** 浏览器设备凭据、密码与短会话管理器，传给只监听 loopback 的代理。 */
  deviceAuth,
  ...
  // 只保留固定域名命名隧道。测试注入继续兼容旧的 startTunnel 名称。
  const startNamed = internals.startNamedTunnel ?? internals.startTunnel ?? startNamedTunnel;
  ...
  // 公网入口开关状态持久化（issue #11）：tunnel-auto.json 记录「公网入口应处于开启状态」。
  // 开启成功时写入；手动关闭（设置页开关、恢复出厂）时清除；dispose/插件重启保留。
  // DSH 启动时检查标记：有 → 自动拉起隧道；无 → 保持关闭。开关状态与 DSH 生命周期解耦。
  ...
  return {
      ...
      for (let p = port; p < port + 10; p++) {
        try {
          proxy = await createProxy({
            ...
            // 不开放局域网监听；公网只能通过本机 cloudflared 进入。
    ...
    async startTunnel() {
      ...
      const onPhase = (phase) => {
        ...
        if (phase === 'downloading') tunnelState.detail = '正在从 Cloudflare 官方源下载并校验 cloudflared（首次约 20-55MB）| downloading official cloudflared with sha256 verification (~20-55MB)';
      ...
      tunnelPromise = (async () => {
        try {
          // 安全模式只接受固定域名命名隧道；没有配置时拒绝开放任何公网入口。
          const cfg = typeof getTunnelConfig === 'function' ? (getTunnelConfig() ?? {}) : {};
          if (!cfg.token || !cfg.hostname) {
            throw new Error(
              '固定公网入口未配置完整：需要 Tunnel Token 和固定域名 | '
              + 'secure public access requires a Tunnel Token and fixed hostname',
            );
          }
          ...
          // 记录「应开启」状态，供重启后自动恢复（issue #11）。
          // 必须写完再返回：若用 void 异步写，紧跟的手动 stopTunnel 可能先删文件、
          // 迟到的写入又把标记复活，把「关闭」覆盖成「开启」。
          await persistAutoTunnel();
          try { onTunnelReady?.('named'); } catch { /* 忽略 */ }
          return tunnel.url;
        } catch (err) {
    ...
    /** 停止公网隧道（代理保持）。手动关闭清除「应开启」标记；dispose/插件重启传 keepAutoState 保留，重启后按标记自动恢复。 */
    stopTunnel({ keepAutoState = false } = {}) {
      tunnelAbort?.abort();
      tunnelAbort = null;
      tunnelPromise = null;
      tunnelFlight = null; // 丢弃已 abort 的 in-flight；旧任务 finally 不会清掉新任务
      if (tunnel) tunnel.kill();
      tunnel = null;
      tunnelState.phase = 'idle';
      tunnelState.detail = '';
      tunnelState.startedAt = null;
      if (!keepAutoState) void clearAutoTunnel(); // 手动关闭 → 记住「关闭」，下次启动不自动开启
    },
    ...
    /** 停止一切（插件卸载时）。保留「应开启」标记：这只是 DSH 退出，不是用户关闭公网入口。 */
    async dispose() {
      this.stopTunnel({ keepAutoState: true });
      if (proxy) {
        const p = proxy;
        proxy = null;
