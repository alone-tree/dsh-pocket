...
import { deviceLoginPage, devicePairPage, SESSION_ACTIVITY_SCRIPT } from './auth-pages.mjs';
 ...
 * @param {object|false} [opts.rateLimit] PIN 登录速率限制参数覆盖（issue #40；测试用短窗口）
 * @param {object} [opts.deviceAuth] Named Tunnel 的设备认证管理器
 * @param {(host:string)=>'named'|'quick'|'reject'|null} [opts.publicChannelForHost] 公网 Host 的严格通道判定
 ...
 */
export function createPocketProxy({ port = 3081, host = '0.0.0.0', upstream = DEFAULT_UPSTREAM, log = null, injectHtml = DEFAULT_INJECT, auth = null, deviceAuth = null, publicChannelForHost = null, rateLimit = null, heartbeat = {}, lanAccessEnabled = () => true, launchToken = () => '', handshakeLimit } = {}) {
  ...
  const server = createServer((req, res) => {
    ...
    const isPublic = hostClass === 'public';
    const publicChannel = isPublic && typeof publicChannelForHost === 'function'
      ? publicChannelForHost(host)
      : (isPublic ? 'quick' : null);
    const requestPath = new URL(req.url ?? '/', 'https://dsh-pocket.invalid').pathname;
    // connection.rpc.handle 不支持 endpoint 权限参数，本机设置必须在改写 Host 之前拦截。
    if (requestPath.startsWith('/dsh-pocket-admin') && hostClass !== 'loopback') {
      res.writeHead(403, { 'content-type': 'application/json', 'cache-control': 'no-store' });
      res.end('{"error":"local-admin-only"}');
      return;
    }
    // Named 模式下陌生公网 Host 不得回落到共享 PIN。
    if (isPublic && publicChannel === 'reject') {
      const html = isHtmlRequest(req);
      res.writeHead(403, { 'content-type': html ? 'text/html; charset=utf-8' : 'application/json', 'cache-control': 'no-store' });
      res.end(html ? '<!doctype html><meta charset="utf-8"><p>公网地址与当前启用通道不匹配 | public host does not match the active channel</p>' : '{"error":"public-host-rejected"}');
      return;
    }
    ...
    // Named Tunnel 使用独立设备认证。只匹配设置中配置的固定域名；LAN、Quick、CLI
    // 继续走原有 PIN 路径。认证端点不会向 DSH 上游透传。
    const useDeviceAuth = Boolean(deviceAuth && publicChannel === 'named');
    if (useDeviceAuth) {
      ...
      if (!deviceAuth.authorizeRequest(req)) {
        if (req.method === 'GET' && isHtmlRequest(req)) {
          const authStatus = deviceAuth.status();
          res.writeHead(200, AUTH_PAGE_HEADERS);
          res.end(deviceLoginPage({ deviceCount: authStatus.deviceCount, paired: deviceAuth.hasApprovedDevice(req) }));
        } else {
    ...
    // 访问令牌认证（issue #13 + #18 + #33 + #40 + #69）：LAN/Quick 使用；Named 由上方设备认证接管。
    if (auth && !useDeviceAuth) {
    ...
    // Named 页面必须保持未压缩，才能注入真实用户活动监听器。
    if (useDeviceAuth && isHtmlRequest(req)) delete headers['accept-encoding'];
    const requestInjectHtml = useDeviceAuth ? `${injectHtml ?? ''}${SESSION_ACTIVITY_SCRIPT}` : injectHtml;
    ...
    const proxyReq = httpRequest(
      ...
      (proxyRes) => {
        ...
        if (requestInjectHtml && contentType.includes('text/html') && !isCompressed(proxyRes.headers)) {
          const chunks = [];
          proxyRes.on('data', (c) => chunks.push(c));
          proxyRes.on('end', () => {
            let html = Buffer.concat(chunks).toString('utf8');
            if (!html.includes(INJECT_MARK)) {
              html = html.replace(/<head[^>]*>/i, (m) => `${m}${requestInjectHtml}`);
            }
  ...
  server.on('upgrade', (req, socket, head) => {
    ...
    try { requestPath = new URL(requestPath, 'https://dsh-pocket.invalid').pathname; } catch {}
    if ((requestPath.startsWith('/dsh-pocket-admin') && hostClass !== 'loopback') || (isPublic && publicChannel === 'reject')) {
    ...
    const useDeviceAuth = Boolean(deviceAuth && publicChannel === 'named');
    if (useDeviceAuth && !deviceAuth.authorizeRequest(req)) {
      socket.write('HTTP/1.1 401 Unauthorized\r\nConnection: close\r\n\r\n');
      socket.destroy();
      return;
    }
    // WebSocket 同样校验（防止绕过 HTTP 认证从 WS 进入；Named 使用短设备会话）。
    if (auth && !useDeviceAuth) {
    ...
    // WS 心跳不能续期；会话超过真实用户活动时限后主动断开既有连接。
    const deviceSessionTimer = useDeviceAuth ? setInterval(() => {
      if (!deviceAuth.authorizeRequest(req)) socket.destroy();
    }, 30_000) : null;
    deviceSessionTimer?.unref?.();
    if (deviceSessionTimer) socket.once('close', () => clearInterval(deviceSessionTimer));
    const headers = loopbackAuthority({ ...req.headers }, upstream);
    const proxyReq = httpRequest({
