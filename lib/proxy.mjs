...
// Cloudflare Named Tunnel；所有页面、API 和 WebSocket 都先经过 Passkey 设备白名单。
//
...
const PASSKEY_CLIENT = readFileSync(new URL('../client/passkey.js', import.meta.url), 'utf8');
...
/** 单次入口票据只供代理消费，绝不能继续传给 DSH 上游或进入其日志。 */
export function stripPocketEntry(reqUrl) {
  try {
    const url = new URL(reqUrl ?? '/', 'http://dsh.invalid');
    if (!url.searchParams.has('pocket-entry')) return reqUrl ?? '/';
    url.searchParams.delete('pocket-entry');
 ...
 * @param {object} [opts.deviceAuth] Passkey/WebAuthn 设备白名单与短会话管理器
...
export function createPocketProxy({ port = 3081, host = '127.0.0.1', upstream = DEFAULT_UPSTREAM, log = null, injectHtml = DEFAULT_INJECT, deviceAuth = null, heartbeat = {}, launchToken = () => '' } = {}) {
  ...
  const server = createServer((req, res) => {
    ...
    if (deviceAuth) {
      if (req.method === 'GET' && requestPath === '/pocket-auth/client.js') {
        res.writeHead(200, {
          'content-type': 'application/javascript; charset=utf-8',
          'cache-control': 'public, max-age=31536000, immutable',
          'x-content-type-options': 'nosniff',
        });
        res.end(PASSKEY_CLIENT);
        return;
      }
      ...
      // 每次顶层页面打开/刷新都必须消耗一次登录后生成的单次入口票据。
      // 资源、API 与 WebSocket 则复用当前页面的短会话。
      if (req.method === 'GET' && isHtmlRequest(req)) {
        if (!deviceAuth.consumeEntryTicket(req)) {
          const status = deviceAuth.status();
          res.writeHead(200, AUTH_PAGE_HEADERS);
          res.end(deviceLoginPage({ credentialCount: status.credentialCount }));
          return;
        }
      } else if (!deviceAuth.authorizeRequest(req)) {
    ...
    const upstreamPath = upstreamPathWithLaunchToken(
      stripDesktopMarkers(stripPocketEntry(req.url)),
      req.method,
      req.headers.cookie,
      launchTok,
    );
